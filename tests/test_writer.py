...
def _meta(item_count: int = 2) -> FeedMeta:
    return FeedMeta(
...
def test_writes_rows_as_one_json_object_per_line(tmp_path: Path):
    ...
    assert len(lines) == 2
    parsed = [FeedRow.model_validate(loads(line)) for line in lines]
    assert parsed == _rows()
...
def test_writes_meta_as_valid_json(tmp_path: Path):
    ...
    parsed = FeedMeta.model_validate(
        loads((tmp_path / "feed-meta.json").read_text(encoding="utf-8"))
    )
    assert parsed == _meta()


def test_output_is_byte_identical_across_runs(tmp_path: Path):
    # Same input twice → same bytes. Pydantic field order is deterministic
    # (alphabetical by declaration) so output never drifts on equal input.
    dir_a = tmp_path / "a"
    dir_b = tmp_path / "b"
    dir_a.mkdir()
    dir_b.mkdir()

    write_feed(_rows(), _meta(), dir_a)
    write_feed(_rows(), _meta(), dir_b)
...
def test_atomic_write_leaves_previous_file_intact_on_failure(tmp_path: Path):
    ...
    with patch("gha_sec_feed.writer.replace", side_effect=OSError("disk full")):
        with pytest.raises(OSError, match="disk full"):
            write_feed(_rows()[:1], _meta(item_count=1), tmp_path)
